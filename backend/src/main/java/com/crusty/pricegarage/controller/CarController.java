...
public class CarController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Car> editCar(@PathVariable Long id, @RequestBody CarRequest request){
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        return ResponseEntity.ok(carService.updateCar(id, request, username));
    }
}
